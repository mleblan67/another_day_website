...
async function load_data(itemId,itemColor){
    ...
    .then(function(data){
            ...
            document.getElementById("price").innerHTML = "$"+(itemJson.price/100)+".00 USD"
...
window.onload = function(){
    ...
    for (var i=0, len=btns.length; i<len; i++) {
        btns[i].onclick = function() {select_size(this)};
    }
    //assign an onchange function to color selector
    document.getElementById('color_select').onchange = function(){select_color(this)};
    ...
    document.getElementById('order_button').onclick = function(){
        if(item.size==null){
            document.getElementById('size_validator').innerText = "*Please choose a size";
        }
        else{
            document.getElementById('size_validator').innerText = "";
            var product = [item.id,item.color,item.size,item.price,item.product,item.shipping];
